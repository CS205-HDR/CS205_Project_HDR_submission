...
def run_AHE_Interpolation_cl(img,clfile = 'AHE_interp_cl.cl',segmentationsize=12,suppressprint=True):
    '''
...
if __name__ == '__main__':
    print 'Started... 6. AHE, opencl, interpolation, buffer'

    out_orig = run_AHE_Interpolation_cl(im_orig)
    gshow(out_orig)
    plt.show()
    hist_np(out_orig)
    cumhist_np(out_orig)

    out_small = run_AHE_Interpolation_cl(im_small)