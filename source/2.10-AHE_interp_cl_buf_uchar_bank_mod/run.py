...
def run_AHE_Interpolation_buffer_smaller_256_bank_cl_mod(img,clfile = 'AHE_interp_cl_buf_uchar_bank_mod.cl',segmentationsize=12,suppressprint=True):
    '''
...
if __name__ == '__main__':
    print 'Started... 10. AHE interpolation, opencl, buffer, float->uchar, avoid bank conflict, faster buffering'

    out_orig = run_AHE_Interpolation_buffer_smaller_256_bank_cl_mod(im_orig)
    gshow(out_orig)
    plt.show()
    hist_np(out_orig)
    cumhist_np(out_orig)

    out_small = run_AHE_Interpolation_buffer_smaller_256_bank_cl_mod(im_small)