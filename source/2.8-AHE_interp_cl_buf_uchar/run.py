...
def run_AHE_Interpolation_buffer_smaller_256_cl(img,clfile = 'AHE_interp_cl_buf_uchar.cl',segmentationsize=12,suppressprint=True):
    '''
...
if __name__ == '__main__':
    print 'Started... 8. AHE interpolation, opencl, buffer, float->uchar'

    out_orig = run_AHE_Interpolation_buffer_smaller_256_cl(im_orig)
    gshow(out_orig)
    plt.show()
    hist_np(out_orig)
    cumhist_np(out_orig)

    out_small = run_AHE_Interpolation_buffer_smaller_256_cl(im_small)